...
contract BusStation {
    ...
    function buyBusTicket(uint256 routeId) external payable canPurchaseTicket(routeId) {
    ...
    modifier canPurchaseTicket(uint256 routeId) {
        ...
        require(msg.value > routes[routeId]._minTicketValue, "Need to pay more for ticket.");
        _;
    }

    // Bus can ride if timelock is passed and tickets exceed reserve price
    modifier isReadyToRide(uint256 routeId) {
        require(routes[routeId]._endOfTimelock <= block.timestamp, "Function is timelocked.");
        require(routes[routeId]._hasBusLeft == false, "Bus is already gone.");
        require(routeBalance[routeId] >= routes[routeId]._minWeiToLeave, "Not enough wei to leave.");
        _;
    }
}
